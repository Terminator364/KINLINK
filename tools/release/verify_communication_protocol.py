...
def main() -> int:
    ...
    require(p.get("schema") == "kinlink.communication_protocol/2",
            "communication protocol: v2 schema missing")
    ...
    require(p.get("backup_earliest_offset_minutes", 0) > 25,
            "communication protocol: backup must not preempt normal close")
    require(p.get("hard_guard_offset_minutes", 0) >
            p.get("backup_earliest_offset_minutes", 0),
            "communication protocol: hard guard must trail shadow backup")
    require(p.get("threading") == "END_REPLY_TO_START_SAME_GMAIL_THREAD",
            "communication protocol: END must stay in START thread")
    ...
    require("NEVER_USE_USER_VISIBLE_RECOVERY_OR_RATTRAPAGE_SUBJECT" in backup,
            "communication protocol: user-visible backup wording fence missing")

    require(sm.get("schema") == "kinlink.communication_state_machine/2",
            "communication state machine: v2 schema missing")
    ...
    require(a.get("schema") == "kinlink.active_tranche/2",
            "communication protocol: active tranche v2 missing")
    ...
    require(lines, "communication ledger: empty")
    # Ensure every line remains valid JSON and current START receipt exists.
    ...
    print("communication-protocol-v2: PASS")
    return 0
