...
class AutopilotRecoveryController(
    ...
    private fun execute(decision: AutomaticRecoveryDecision, triggeringTruth: NetworkTruth) {
        ...
        val network = cm.activeNetwork ?: run {
            record(false, "NO_NETWORK", "Réseau disparu avant l’action.", started)
            return
        }
        val caps = cm.getNetworkCapabilities(network) ?: run {
            record(false, "NO_CAPS", "Capacités réseau indisponibles.", started)
            return
        }
        if (!caps.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)) {
            record(false, "ABORT_NON_WIFI", "Transport changé : aucune action hors Wi-Fi.", started)
            return
        }
        if (caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_CAPTIVE_PORTAL)) {
            record(false, "ABORT_PORTAL", "Portail captif : intervention utilisateur requise.", started)
            return
        }
        ...
        when (decision.action) {
            AutomaticRecoveryAction.REFRESH_METRICS -> {
                if (!stillSameActiveWifi(network)) {
                    record(false, "WATCHDOG_TRANSPORT_CHANGED", "Transport changé avant refresh; abandon fail-open.", started)
                    return
                }
                if (watchdogExpired(started)) {
                    record(false, "WATCHDOG_TIMEOUT", "Deadline de récupération dépassée avant refresh; abandon fail-open.", started)
                    return
                }
                val refreshed = runCatching { cm.requestBandwidthUpdate(network) }.getOrDefault(false)
                record(refreshed, "REFRESH", "Wi-Fi validé mais instable : métriques rafraîchies sans probe.", started)
                if (refreshed) onRecoveryActionExecuted?.invoke(triggeringTruth)
            }
            AutomaticRecoveryAction.CONFIRM_WIFI -> {
                if (caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED)) {
                    val refreshed = runCatching { cm.requestBandwidthUpdate(network) }.getOrDefault(false)
                    record(true, "ALREADY_VALIDATED", "Android a validé Internet avant le probe; aucune sonde envoyée. Refresh=$refreshed", started)
                    return
                }
                val metered = !caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_NOT_METERED)
                if (metered) {
                    record(false, "ABORT_METERED_WIFI", "Wi-Fi mesuré détecté : aucun micro-probe automatique.", started)
                    return
                }
                ...
                if (!stillSameActiveWifi(network)) {
                    record(false, "WATCHDOG_TRANSPORT_CHANGED", "Transport changé pendant le micro-probe; aucune action réseau supplémentaire.", started)
                    return
                }
                val watchdogAction = RecoveryWatchdogPolicy.action(
                    nowElapsedMillis = SystemClock.elapsedRealtime(),
                    lastHeartbeatElapsedMillis = started,
                    hardDeadlineMillis = RECOVERY_DEADLINE_MS
                )
                if (watchdogAction == RecoveryWatchdogAction.FREEZE_NEW_ACTIONS) {
                    record(
                        false,
                        "WATCHDOG_FREEZE",
                        "Budget de temps à moitié consommé après micro-probe; aucun refresh supplémentaire.",
                        started
                    )
                    return
                }
    ...
    private fun record(
        ...
        suffix: String,
        summary: String,
        startedElapsedMillis: Long? = null
    ) {
        val duration = startedElapsedMillis?.let {
            (SystemClock.elapsedRealtime() - it).coerceAtLeast(0L)
        }
        runCatching {
            ledger.appendAction(
                "${ACTION_PREFIX}_$suffix",
                success,
                summary,
